"""Binary sensor platform for NOMOS devices."""
...
from .models import DEVICE_TYPES, NomosBinarySensorDescriptor
...
async def async_setup_entry(
    ...
    """Set up NOMOS binary sensors for a config entry."""
    device_type = DEVICE_TYPES[entry.data[CONF_DEVICE_TYPE]]
    async_add_entities(
        NomosBinarySensor(entry, descriptor) for descriptor in device_type.binary_sensors
    )


class NomosBinarySensor(BinarySensorEntity):
    """A binary sensor whose value comes from a NOMOS device's MQTT state topic."""

    _attr_should_poll = False
    _attr_has_entity_name = True

    def __init__(self, entry: ConfigEntry, descriptor: NomosBinarySensorDescriptor) -> None:
        """Initialize the binary sensor from its descriptor."""
    ...
    async def async_added_to_hass(self) -> None:
        ...
        def message_received(msg: mqtt.ReceiveMessage) -> None:
            ...
            self._attr_is_on = bool(payload[self._descriptor.json_key])
